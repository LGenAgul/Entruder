...
def prepare_session(tenant, client_id, service):
    """Shared preamble for every enum command: resolve tenant/client from the
    passed args or the cache, ensure a valid `service` session ("graph" or
    "management"), and return (tenant, headers) ready for request_json.

    Explicitly-passed -tenant/-clientid are honored and written back to the
    cache; otherwise the cached values are used.
    """
...
def graph_collect(url, headers, params=None, delegated=False):
    """Follow @odata.nextLink paging and return the merged `value` list.

    Set delegated=True for /me endpoints so the failure hint points the user at
    logging in with a user session (app-only tokens have no /me).
    """
    items = []
    while url:
        vprint(f"GET {url}")
...
def _project_storage(acct):
    ...
    return {
        "id":              acct.get("id"),
        "name":            acct.get("name"),
        "resource_group":  _resource_group(acct.get("id")),
...
def enum_owned(
    ...
    identity. Requires a delegated (user) session; /me is empty for app-only tokens."""
    tenant, headers = prepare_session(tenant, client_id, "graph")
...
def enum_storage_accounts(
    ...
    if check_access:
        ...
        accounts = [_annotate_storage_access(headers, storage_headers, a) for a in accounts]
...
def enum_containers(
    ...
    always needs a valid storage token — log in first (secret/cert/ropc/etc
    against the storage resource) and pass -tenant/-clientid, or rely on the
...
def enum_blobs(
    ...
    anonymous/public exposure directly — this succeeds without a token when
    the container's public access level is set to "Container" (full public
    read)."""
    headers = _storage_headers(tenant, client_id)
    endpoint = f"https://{account}.blob.core.windows.net"
